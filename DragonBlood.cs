...
public class DragonBlood
{
    ...
    {
        //Heritage Feat
        Feat dragonBloodHeritage = new Feat(ModData.FeatNames.DragonBlood,
                ...
                "You can choose from dragonblood feats and feats from your ancestry whenever you gain an ancestry feat.",[ModData.Traits.Dragonblood, Trait.VersatileHeritage],null)
    ...
    private static void CreateMagicalDragonbloodLogic(TrueFeat magicBlood)
    {
        magicBlood.WithPrerequisite(
            ...
            .WithOnCreature(self =>
            {
                self.GetOrCreateSpellcastingSource(SpellcastingKind.Innate, ModData.Traits.Dragonblood, Ability.Charisma, DetermineTraitMagic(magicBlood)).WithSpells(
    ...
    private static void CreateTraditionalResistancesLogic(TrueFeat traditionalResistances)
    {
        traditionalResistances.WithPrerequisite(
            ...
            .WithOnCreature((sheet, self) =>
            {
                self.AddQEffect(new QEffect("Traditional Resistances", "You gain a +1 status bonus to AC and saves against spells and other magical effects from the same tradition as your lineage. This bonus increases to +2 against sleep and paralysis effects.")
                    {
                        BonusToDefenses = (_, action, _) =>
                    {
                        ...
                        {
                            return new Bonus(action.HasTrait(Trait.Sleep) || action.SpellId is SpellId.Paralyze ? 2 : 1, BonusType.Status, "Traditional Resistances");
                        }
                        if (action != null 
    ...
    private static IEnumerable<Feat> DraconicAspectFeats()
    {
        ...
        {
            Item tail = new Item(ModData.Illustrations.DragonTail, "tail",
                Trait.Sweep, Trait.Trip, Trait.Unarmed, Trait.Brawling).WithWeaponProperties(new WeaponProperties("1d6", DamageKind.Bludgeoning));
            if (self.HasFeat(ModData.FeatNames.DeadlyAspect))
